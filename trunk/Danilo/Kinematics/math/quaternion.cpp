...
Quaternion::Quaternion(double thetaX, double thetaY, double thetaZ)
{
...
{
    double radius = AxisAngle.x4*M_PI/360.0;
    double scale_ = cos(radius);
...
double Quaternion::toAngle()
{
    double angle = acos(this->getScalar()) * 2.0;
    return angle;
}
...
Matrix4f Quaternion::getMatrix()
{
        ...
        double x2 = this->getPosX() * this->getPosX();
        double y2 = this->getPosY() * this->getPosY();
        double z2 = this->getPosZ() * this->getPosZ();
        double xy = this->getPosX() * this->getPosY();
        double xz = this->getPosX() * this->getPosZ();
        double yz = this->getPosY() * this->getPosZ();
        double wx = this->getScalar() * this->getPosX();
        double wy = this->getScalar() * this->getPosY();
        double wz = this->getScalar() * this->getPosZ();

        // This calculation would be a lot more complicated for non-unit length quaternions
        // Note: The constructor of Matrix4 expects the Matrix in column-major format like expected by
        //   OpenGL
//        double rot[16] = {1.0f - 2.0f * (y2 + z2), 2.0f * (xy - wz), 2.0f * (xz + wy), 0.0f,
...
Quaternion Quaternion::fromRotation(Quaternion q){
    ...
    double sinhalftheta;
    double axisnorm;
    double n = q.getPosX()*q.getPosX()+q.getPosY()*q.getPosY()+q.getPosZ()*q.getPosZ();
